...
import { sameOrigin,verifyPin,staffSession,sessionCookie,json } from './_shared/staff-session.mts';
...
export default async(req,context)=>{
 ...
 try{
 const pinHash=Netlify.env.get('STAFF_PIN_HASH');if(!pinHash)return json({error:'El acceso por NIP aún no está configurado.'},503);
 ...
 const body=await req.json();if(!verifyPin(body.pin))return json({error:'NIP incorrecto.'},401);
